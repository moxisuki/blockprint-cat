...
class FormatCatalogTest {
    ...
    @Test
    fun convertTargetsExcluding_partialNbt_source_returns_all_four_valid_targets() {
        // PartialNbt is NOT in the writable list (it's a read-side category),
        // so passing it as current just means we offer all 4 writable targets.
    ...
    @Test
    fun convertTargetsExcluding_buildingHelper_source_omits_buildingHelper() {
    ...
    @Test
    fun convertTargetsExcluding_unknown_source_returns_all_four_valid_targets() {
        // Unknown is NOT a valid convert target itself, but as a source we still
        // offer the user all 4 writable formats.
